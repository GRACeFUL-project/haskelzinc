...
import Interfaces.MonVar (KM, safeExtract)
import Interfaces.MonPrint (layout, pprint)
import Interfaces.FZSolutionParser (Solution, tryDefaultSolutions, getSolutions, getDefaultSolutionsFromFile)
...
iRunModel :: KM a -> IO (Either ParseError [Solution])
iRunModel m = do
...
runModel = runModelWithParser tryDefaultSolutions
...
-- default.
runModelWithParser :: (Int -> Parser [Solution]) -- ^ The parser with which solutions will be
                                                 -- parsed
                    -> KM a                      -- ^ The model
                    ...
                    -> Int                       -- ^ The number of solutions to be returned
                    -> IO (Either ParseError [Solution])
runModelWithParser p m mpath s n = do
  ...
  writeFile (mpath ++ ".mzn") (layout $ safeExtract m)
  readCreateProcess (shell mfzn) (layout $ safeExtract m)
  res <- case s of
  ...
  getDefaultSolutionsFromFile (mpath ++ ".results.txt") n
